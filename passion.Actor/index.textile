...
* "update and draw":{{ site.root_path }}/passion.Actor/update-and-draw methods. These methods are empty by default. Modifying them is the easiest way to alter how an Actor behaves / looks like. These functions can be redefined depending on the actor's state.
* "input handling":{{ site.root_path }}/passion.Actor/processing-input
...
* two class methods("apply and applySorted":{{ site.root_path }}/passion.Actor/apply) for managing groups of actors
